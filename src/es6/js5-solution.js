...
const filter_hasInvalidEmail = (person) => ('email' in person) ? (!isValidEmail(person.email)) : true;
...
const addSpaceToString = (string, position) => string.slice(0, position) + " " + string.slice(position);
